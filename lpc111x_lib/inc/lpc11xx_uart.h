 ...
 * 				support for UART firmware library on LPC11xx
 * @version	: 1.0
 * @date	: 21. Jan. 2010
 * @author	: Coocox
 **************************************************************************/
...
#ifndef __LPC11XX_UART_H
#define __LPC11XX_UART_H
...
#define UART_MCR_BITMASK		((uint8_t)(0x0D3))		/*!< UART bit mask value */
...
/* Macro defines for UART Modem (UART only) status register */
...
/* Macro defines for UART RS-485 Address Match register */
#define UART_RS485ADRMATCH_BITMASK ((uint8_t)(0xFF)) 	/**< Bit mask value */

/* Macro defines for UART RS-485 Delay value register */
...
//	UART_INTCFG_MS,		/*!< Modem status interrupt enable (UART only) */
//	UART_INTCFG_CTS,		/*!< CTS1 signal transition interrupt enable (UART only) */
...
/* Macro to check the input UART_SignalState parameters */
#define PARAM_UART_SIGNALSTATE(x) ((x==INACTIVE) || (x==ACTIVE))
...
/** Macro to check the input PARAM_UART_MODEM_PIN parameters */
#define PARAM_UART_MODEM_PIN(x) ((x==UART_MODEM_PIN_DTR) || (x==UART_MODEM_PIN_RTS))
...
/** Macro to check the input PARAM_UART_MODEM_MODE parameters */
#define PARAM_UART_MODEM_MODE(x) ((x==UART_MODEM_MODE_LOOPBACK) || (x==UART_MODEM_MODE_AUTO_RTS) \
								|| (x==UART_MODEM_MODE_AUTO_CTS))
...
#define PARAM_UART_RS485_DIRCTRL_PIN(x)	((x==UART_RS485_DIRCTRL_RTS) || (x==UART_RS485_DIRCTRL_DTR))
...
/********************************************************************//**
* @brief UART Full modem -  RS485 Control configuration type
**********************************************************************/
typedef struct {
...
/** UART modem status interrupt callback type */
...
#define PARAM_UARTx(x)	((uint32_t *)x)==((uint32_t *)LPC_UART)
#define PARAM_UART_MODEM(x) (((uint32_t *)x)==((uint32_t *)LPC_UART))
