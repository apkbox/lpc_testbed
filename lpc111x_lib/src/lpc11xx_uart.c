/***********************************************************************//**
 * @file	: lpc11xx_uart.c
 * @brief	: Contains all functions support for UART firmware library on LPC11xx
 * @version	: 1.0
 * @date	: 22. Jan. 2010
 * @author	: Coocox
 **********************************************************************/
...
/** UART modem status interrupt callback pointer data */
...
Status uart_set_divisors(LPC_UART_TypeDef *UARTx, uint32_t baudrate)
{
	...
	* 0 < mulFracDiv <= 15, 0 <= dividerAddFracDiv <= 15 */
	...
	if (relativeOptimalError < ((baudrate * UART_ACCEPTED_BAUDRATE_ERROR)/100))
	{
    	...
    	((LPC_UART_TypeDef *)UARTx)->DLM = UART_LOAD_DLM(diviserOptimal);
    	((LPC_UART_TypeDef *)UARTx)->DLL = UART_LOAD_DLL(diviserOptimal);
    	/* Then reset DLAB bit */
    	((LPC_UART_TypeDef *)UARTx)->LCR &= (~UART_LCR_DLAB_EN) & UART_LCR_BITMASK;
    	((LPC_UART_TypeDef *)UARTx)->FDR = (UART_FDR_MULVAL(mulFracDivOptimal) \
...
void UART_GenIntHandler(LPC_UART_TypeDef *UARTx)
{
	...
	/*
	 * In case of using UART with full modem,
	 * interrupt ID = 0 that means modem status interrupt has been detected
	 */	
	if (tmp == 0){
		// Check Modem status
		modemsts = UARTx->MSR & UART_MSR_BITMASK;
		...
		//else {
		//	UARTx->IER &= ~(UART_IER_MSINT_EN | UART_IER_CTSINT_EN);
		//}
	}
 ...
 * @param[in]	UARTx	UART peripheral selected, should be UART0, UART,
 * 						UART2 or UART3.
 * @return 		None
 **********************************************************************/
void UART_DeInit(LPC_UART_TypeDef* UARTx)
{
	...
	SYSCON_AHBPeriphClockCmd (SYSCON_AHBPeriph_UART, DISABLE);
}

/********************************************************************//**
 * @brief 		Init UART pins, pins assign:
 *                UART RXD - PIO1_6
 *                UART TXD - PIO1_7 
 *
 * @return		None
...
/********************************************************************//**
 * @brief 		Init UART mode pins, pins assign:
 *                UART DTR - PIO2_0
 *                UART CTS - PIO0_7
 *                UART RTS - PIO1_5
 *                UART DSR - PIO2_1
 ...
 * @return		None
 *********************************************************************/
void UART_ModePinsInit(void)
{
    IOCON_SetPinFunc(IOCON_PIO2_0, PIO2_0_FUN_DTR);   /* UART DTR - PIO2_0 */
    IOCON_SetPinFunc(IOCON_PIO0_7, PIO0_7_FUN_CTS);   /* UART CTS - PIO0_7 */
    IOCON_SetPinFunc(IOCON_PIO1_5, PIO1_5_FUN_RTS);   /* UART RTS - PIO1_5 */
    
    /* DSR_PIO2_1  */     
    /* DSR_PIO3_1  */
    IOCON_DSRLocate(DSR_PIO2_1);                      
    IOCON_SetPinFunc(IOCON_PIO2_1, PIO2_1_FUN_DSR);   /* UART DSR - PIO2_1 */

    //IOCON_DSRLocate(DSR_PIO3_1);                      
    //IOCON_SetPinFunc(IOCON_PIO3_1, PIO3_1_FUN_DSR);  
    
    /* DCD_PIO2_2 */
    /* DCD_PIO3_2 */
    IOCON_DCDLocate(DCD_PIO2_2);
    IOCON_SetPinFunc(IOCON_PIO2_2, PIO2_2_FUN_DCD);   /* UART DCD - PIO2_2 */

    //IOCON_DCDLocate(DCD_PIO3_2);
    //IOCON_SetPinFunc(IOCON_PIO3_2, PIO2_3_FUN_DCD);

    /* RI_PIO2_3 */
    /* RI_PIO3_3 */
    IOCON_RILocate(RI_PIO2_3);
    IOCON_SetPinFunc(IOCON_PIO2_3, PIO2_3_FUN_RI);    /* UART RI - PIO2_3 */

    //IOCON_RILocate(RI_PIO3_3);
    //IOCON_SetPinFunc(IOCON_PIO3_3, PIO3_3_FUN_RI);
}
...
void UART_Init(LPC_UART_TypeDef *UARTx, UART_CFG_Type *UART_ConfigStruct)
{
    ...
    // If UART_PCLK is shut down, start it
    if (SYSCON_GetUARTClockDiv() == 0) {
        SYSCON_SetUARTClockDiv(1);
    }    

	/* FIFOs are empty */
	((LPC_UART_TypeDef *)UARTx)->FCR = ( UART_FCR_FIFO_EN \
			| UART_FCR_RX_RS | UART_FCR_TX_RS);
	// Disable FIFO
	((LPC_UART_TypeDef *)UARTx)->FCR = 0;

	// Dummy reading
	while (((LPC_UART_TypeDef *)UARTx)->LSR & UART_LSR_RDR)
	{
		tmp = ((LPC_UART_TypeDef *)UARTx)->RBR;
	}

	((LPC_UART_TypeDef *)UARTx)->TER = UART_TER_TXEN;
	// Wait for current transmit complete
	while (!(((LPC_UART_TypeDef *)UARTx)->LSR & UART_LSR_THRE));
	// Disable Tx
	((LPC_UART_TypeDef *)UARTx)->TER = 0;

	// Disable interrupt
	((LPC_UART_TypeDef *)UARTx)->IER = 0;
	// Set LCR to default state
	((LPC_UART_TypeDef *)UARTx)->LCR = 0;
	// Set ACR to default state
	((LPC_UART_TypeDef *)UARTx)->ACR = 0;
	// Set Modem Control to default state
	((LPC_UART_TypeDef *)UARTx)->MCR = 0;
	// Set RS485 control to default state
	((LPC_UART_TypeDef *)UARTx)->RS485CTRL = 0;
	// Set RS485 delay timer to default state
	((LPC_UART_TypeDef *)UARTx)->RS485DLY = 0;
	// Set RS485 addr match to default state
	((LPC_UART_TypeDef *)UARTx)->ADRMATCH = 0;
	//Dummy Reading to Clear Status
	tmp = ((LPC_UART_TypeDef *)UARTx)->MSR;
	tmp = ((LPC_UART_TypeDef *)UARTx)->LSR;
		
	// Set Line Control register ----------------------------

	uart_set_divisors(UARTx, (UART_ConfigStruct->Baud_rate));

	tmp = (((LPC_UART_TypeDef *)UARTx)->LCR & (UART_LCR_DLAB_EN | UART_LCR_BREAK_EN)) \
			& UART_LCR_BITMASK;
	...
	((LPC_UART_TypeDef *)UARTx)->LCR = (uint8_t)(tmp & UART_LCR_BITMASK);
}
...
void UART_SendData(LPC_UART_TypeDef* UARTx, uint8_t Data)
{
	...
	((LPC_UART_TypeDef *)UARTx)->THR = Data & UART_THR_MASKBIT;
}
...
uint8_t UART_ReceiveData(LPC_UART_TypeDef* UARTx)
{
	...
	return (((LPC_UART_TypeDef *)UARTx)->RBR & UART_RBR_MASKBIT);
}
...
void UART_ForceBreak(LPC_UART_TypeDef* UARTx)
{
	...
	((LPC_UART_TypeDef *)UARTx)->LCR |= UART_LCR_BREAK_EN;
}
...
//			break;
	...
	if (NewState == ENABLE)
	{		
		((LPC_UART_TypeDef *)UARTx)->IER |= tmp;
	}
	else
	{		
		((LPC_UART_TypeDef *)UARTx)->IER &= (~tmp) & UART_IER_BITMASK;
	}
}
...
//	}
	...
	((LPC_UART_TypeDef *)UARTx)->FCR = tmp & UART_FCR_BITMASK;
}
				...
				FunctionalState NewState)
{
	...
	if (NewState == ENABLE)
	{
		...
		((LPC_UART_TypeDef *)UARTx)->LCR |= UART_LCR_DLAB_EN;
		((LPC_UART_TypeDef *)UARTx)->DLL = 0;
		((LPC_UART_TypeDef *)UARTx)->DLM = 0;
		((LPC_UART_TypeDef *)UARTx)->LCR &= ~UART_LCR_DLAB_EN;
		// FDR value must be reset to default value
		((LPC_UART_TypeDef *)UARTx)->FDR = 0x10;
		((LPC_UART_TypeDef *)UARTx)->ACR = UART_ACR_START | tmp;
	}
	else
	{
		((LPC_UART_TypeDef *)UARTx)->ACR = 0;
...
void UART_TxCmd(LPC_UART_TypeDef *UARTx, FunctionalState NewState)
{
	...
	{
		((LPC_UART_TypeDef *)UARTx)->TER &= (~UART_TER_TXEN) & UART_TER_BITMASK;
	}
}
 ...
 * @param[in]	UARTx	UART (only)
 * @param[in]	Pin	Pin that NewState will be applied to, should be:
 * 				- UART_MODEM_PIN_DTR: DTR pin.
 * 				- UART_MODEM_PIN_RTS: RTS pin.
 * @param[in]	NewState New State of DTR/RTS pin, should be:
 * 				- INACTIVE: Force the pin to inactive signal.
				- ACTIVE: Force the pin to active signal.
 * @return none
 **********************************************************************/
void UART_FullModemForcePinState(LPC_UART_TypeDef *UARTx, UART_MODEM_PIN_Type Pin, \
							UART_SignalState NewState)
{
	uint8_t tmp = 0;

	CHECK_PARAM(PARAM_UART_MODEM(UARTx));
	CHECK_PARAM(PARAM_UART_MODEM_PIN(Pin));
	CHECK_PARAM(PARAM_UART_SIGNALSTATE(NewState));

	switch (Pin){
	case UART_MODEM_PIN_DTR:
		tmp = UART_MCR_DTR_CTRL;
		break;
	case UART_MODEM_PIN_RTS:
		tmp = UART_MCR_RTS_CTRL;
		break;
	default:
		break;
	}
	...
	if (NewState == ACTIVE){
		...
		UARTx->MCR &= (~tmp) & UART_MCR_BITMASK;
	}
}
 ...
 * 				- UART_MODEM_MODE_LOOPBACK: Loop back mode.
 * 				- UART_MODEM_MODE_AUTO_RTS: Auto-RTS mode.
 * 				- UART_MODEM_MODE_AUTO_CTS: Auto-CTS mode.
 * @param[in]	NewState New State of this mode, should be:
 * 				- ENABLE: Enable this mode.
				- DISABLE: Disable this mode.
 * @return none
 **********************************************************************/
void UART_FullModemConfigMode(LPC_UART_TypeDef *UARTx, UART_MODEM_MODE_Type Mode, \
							FunctionalState NewState)
{
	uint8_t tmp;

	CHECK_PARAM(PARAM_UART_MODEM(UARTx));
	CHECK_PARAM(PARAM_UART_MODEM_MODE(Mode));
	CHECK_PARAM(PARAM_FUNCTIONALSTATE(NewState));

	switch(Mode){
	case UART_MODEM_MODE_LOOPBACK:
		tmp = UART_MCR_LOOPB_EN;
		break;
	case UART_MODEM_MODE_AUTO_RTS:
		tmp = UART_MCR_AUTO_RTS_EN;
		break;
	case UART_MODEM_MODE_AUTO_CTS:
		tmp = UART_MCR_AUTO_CTS_EN;
		break;
	default:
		break;
	}
 ...
 * 			used to check all flags.
 **********************************************************************/
uint8_t UART_FullModemGetStatus(LPC_UART_TypeDef *UARTx)
{
	CHECK_PARAM(PARAM_UART_MODEM(UARTx));
 ...
 * @return		None
 **********************************************************************/
void UART_RS485Config(LPC_UART_TypeDef *UARTx, UART_RS485_CTRLCFG_Type *RS485ConfigStruct)
{
	uint32_t tmp;
	...
	if (RS485ConfigStruct->AutoAddrDetect_State == ENABLE)
	{
		...
		// Fill Match Address
		UARTx->ADRMATCH = RS485ConfigStruct->MatchAddrValue & UART_RS485ADRMATCH_BITMASK;
	}
 ...
 * 				should be UART0, UART, UART2 or UART3.
 ...
 * 						3 - Error Call-back
 * 						4 - Modem Status Call-back (UART only)
...
/*********************************************************************//**
 * @brief		Standard UART interrupt handler
 * @param[in]	None
 * @return
 **********************************************************************/
void UART_StdIntHandler(void)
{
	UART_GenIntHandler(LPC_UART);
}
